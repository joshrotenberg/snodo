defmodule MCP.Schema.Validator.JSV do
  @moduledoc """
  Optional JSV-backed JSON Schema validation with an offline, non-casting policy.

  Install the separate `mcp_ex_jsv` integration and select this module with
  `use MCP.Server, schema_validator: MCP.Schema.Validator.JSV`.
  ...
  `MCP.Schema.Validator.JSV.BuildError` from `validate/2`, so the MCP router
  ...
  alias MCP.Schema.Validator.JSV.OfflineResolver
  alias MCP.Schema.Validator.JSV.Policy
