defmodule MCP.Transport.Plug do
  @moduledoc """
  ...
  supplies `MCP.Context.auth`; request headers and JSON never supply identity.
  ...
  alias MCP.Transport.Plug.Lease
  alias MCP.Transport.Plug.Stream
  alias MCP.Transport.StreamableHTTP
  alias MCP.Transport.StreamableHTTP.Request
  alias MCP.Transport.StreamableHTTP.Response
  ...
  def init(opts) do
    ...
    unless match?(%Runtime{}, runtime),
      do: raise(ArgumentError, ":runtime must be an MCP.Server.Runtime")
