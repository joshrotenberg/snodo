defmodule MCP.Tool.Simple do
  @moduledoc """
  Opt-in argument DSL for tools with straightforward object input schemas.

  `MCP.Tool.Simple` builds the same raw JSON Schema returned by `MCP.Tool` and
  leaves `call/2` untouched. Argument maps therefore retain protocol-native
  string keys, and simple tools register in `MCP.Router` exactly like raw tools.
      ...
      defmodule Search do
        ...
        def call(%{"query" => query} = arguments, _context) do
          {:ok, MCP.Result.text("searching for \#{query} on page \#{arguments["page"] || 1}")}
        end
      end
  ...
  abandoning the concise form. The raw `MCP.Tool` DSL remains available when
  ...
  defmacro __using__(opts) do
    ...
    quote do
      use MCP.Tool, name: unquote(name), description: unquote(description)

      import MCP.Tool.Simple, only: [argument: 2, argument: 3]

      @mcp_tool_input_schema MCP.Tool.Simple.new_schema!(
  ...
  defmacro argument(name, type, opts \\ []) do
    quote do
      @mcp_tool_input_schema MCP.Tool.Simple.add_argument!(
  ...
  def new_schema!(opts, env) when is_list(opts) do
    validate_options!(opts, @root_options, env, "MCP.Tool.Simple")
    ...
    unless is_map(root_schema) and JSONValue.valid?(root_schema) do
      compile_error!(env, "MCP.Tool.Simple :schema must be a JSON Schema map")
    end
    ...
    unless schema["type"] == "object" and is_map(schema["properties"]) and
      ...
      compile_error!(env, "MCP.Tool.Simple must produce an object schema with properties")
    end
  ...
  def new_schema!(_opts, env) do
    compile_error!(env, "MCP.Tool.Simple options must be a keyword list")
  end
  ...
  def add_argument!(root, name, type, opts, env)
    ...
    if name == "", do: compile_error!(env, "MCP.Tool.Simple argument names cannot be empty")

    validate_options!(opts, @property_options, env, "MCP.Tool.Simple argument #{inspect(name)}")
    validate_property_options!(opts, env, name)

    properties = Map.fetch!(root, "properties")

    if Map.has_key?(properties, name) do
      compile_error!(env, "MCP.Tool.Simple argument #{inspect(name)} is declared more than once")
    end
    ...
    unless JSONValue.valid?(property) do
      compile_error!(env, "MCP.Tool.Simple argument #{inspect(name)} must produce JSON Schema")
    end
  ...
  def add_argument!(_root, name, _type, _opts, env) do
    compile_error!(
      env,
      "MCP.Tool.Simple argument expects a string name and keyword options, got #{inspect(name)}"
    )
  end
  ...
  defp type_schema!(type, env) do
    compile_error!(
      env,
      "MCP.Tool.Simple argument type must be a JSON type atom, {:array, type}, or schema map; got #{inspect(type)}"
    )
  end
  ...
  defp validate_root_options!(opts, env) do
    case Keyword.fetch(opts, :additional_properties) do
      ...
      {:ok, _invalid} ->
        compile_error!(env, "MCP.Tool.Simple :additional_properties must be a schema or boolean")
    end
  end
