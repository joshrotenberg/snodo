defmodule MCP.Subscription.Hub do
  @moduledoc """
  An opt-in, application-supervised `MCP.Subscription.Source`.

  A hub broadcasts protocol-neutral `MCP.Subscription.Event` values to every
  ...
  application's supervision tree and pass `source/1` to `MCP.Server.runtime/1`:

      children = [{MCP.Subscription.Hub, name: MyApp.SubscriptionHub}]
  ...
  `MCP.Subscription.Event.extension/4` values through the same `publish/2`
  ...
  Options are:
    ...
    * `:instrumentation` - an optional `MCP.Instrumentation` sink.
  """
  ...
  @doc "Returns a source configuration suitable for `MCP.Server.runtime/1`."
  ...
  def publish(_hub, _invalid) do
    {:error, {:invalid_event, "subscription producer requires an MCP.Subscription.Event"}}
  end
