defmodule MCP.Resource do
  ...
  defp compile_definition!(env, opts) do
    ...
    kind =
      case {uri, uri_template} do
        {uri, nil} when is_binary(uri) and uri != "" -> :resource
        {nil, template} when is_binary(template) and template != "" -> :template
        _invalid -> compile_error!(env, "MCP.Resource expects exactly one :uri or :uri_template")
      end
  ...
  defp literal_options!(env, opts_ast) do
    if Macro.quoted_literal?(opts_ast) do
      ...
      compile_error!(env, "MCP.Resource options must be compile-time literals")
    end
  end
  ...
  defp validate_definition!(resource, _definition) do
    raise ArgumentError,
          "resource #{inspect(resource)} definition/0 must return MCP.Resource.Definition"
  end
