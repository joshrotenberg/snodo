defmodule MCP.Transport.StreamableHTTP do
  ...
  submitting only application work to `MCP.Server.Executor`.
  ...
  alias MCP.Transport.StreamableHTTP.Prepared
  ...
  def execute(%Runtime{} = runtime, %Prepared{} = prepared, cancellation \\ nil) do
    ...
    case Server.dispatch(runtime, prepared.raw, transport) do
      ...
      {:stream, subscription} ->
        :ok = MCP.Subscription.close(subscription, {:error, :stream_not_allowed})
