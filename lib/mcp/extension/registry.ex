defmodule MCP.Extension.Registry do
  @moduledoc """
  ...
  alias MCP.Extension.Route
  alias MCP.JSONValue
  alias MCP.Protocol.Inspector
  alias MCP.Protocol.Registry, as: ProtocolRegistry
  alias MCP.Result
  alias MCP.Subscription.Event, as: SubscriptionEvent
  alias MCP.Subscription.Filter, as: SubscriptionFilter
  ...
  defp register(%__MODULE__{} = registry, entry, protocol_registry) do
    ...
    unless is_list(methods) and methods != [] and Enum.all?(methods, &match?(%Method{}, &1)) do
      raise ArgumentError,
            "extension #{inspect(id)} must declare at least one MCP.Extension.Method"
    end
