defmodule MCP.Client.HTTP do
  @moduledoc """
  Streamable HTTP transport for `MCP.Client.connect({:http, url}, opts)`.
  ...
  whatever the HTTP status, so `MCP.Client` decodes it as `{:error,
  %MCP.Error{}}`. Anything else is a -32000 transport error with the status and
