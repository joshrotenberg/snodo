defmodule Snodo.MixExampleTest do
  use ExUnit.Case, async: false

  alias Mix.Tasks.Snodo.Example

  @environment_keys ["MIX_ENV", "MIX_BUILD_PATH", "SNODO_EXPECTED_BUILD_PATH"]
  ...
  test "child keeps the resolved project environment and build path without MIX_ENV" do
    ...
    System.put_env("SNODO_EXPECTED_BUILD_PATH", build_path)
    ...
    for inherited_path <- [build_path, Path.join([build_path, "..", Path.basename(build_path)])] do
      ...
      Mix.Task.run("snodo.example", [fixture("environment.fixture")])
  ...
  test "invalid arguments and missing files fail before starting a child" do
    assert_raise Mix.Error, "usage: mix snodo.example PATH", fn ->
      Example.run([])
    end
