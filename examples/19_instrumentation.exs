...
defmodule Examples.Instrumentation.Echo do
  ...
  use MCP.Tool, name: "observed_echo"
...
defmodule Examples.Instrumentation.Runner do
  ...
  defp exercise(runtime, hub) do
    ...
    assert_event!([:mcp_ex, :server, :dispatch, :start], fn measurements, metadata ->
      is_integer(measurements.system_time) and metadata.method == "tools/list"
    end)

    assert_event!([:mcp_ex, :server, :dispatch, :stop], fn measurements, metadata ->
    ...
    assert_event!([:mcp_ex, :server, :dispatch, :start], fn _measurements, metadata ->
      metadata.request_id == "observed-sub"
    end)

    assert_event!([:mcp_ex, :subscription, :open], fn measurements, metadata ->
      measurements.subscriptions == 1 and metadata.filter_keys == ["toolsListChanged"]
    end)

    assert_event!([:mcp_ex, :server, :dispatch, :stop], fn _measurements, metadata ->
      metadata.outcome == :stream
    end)

    {:ok, %{dropped: 0}} = Hub.notify_tools_list_changed(hub)
    assert_event!([:mcp_ex, :subscription, :publish], &no_drop?/2)

    {:ok, %{dropped: 1}} = Hub.notify_tools_list_changed(hub)
    assert_event!([:mcp_ex, :subscription, :publish], &one_drop?/2)
    assert_event!([:mcp_ex, :subscription, :overflow], &oldest_overflow?/2)

    :ok = Hub.complete(hub)

    assert_event!([:mcp_ex, :subscription, :complete], fn measurements, _metadata ->
      measurements.subscriptions == 1 and measurements.queued == 1
    end)

    :ok = Subscription.close(subscription, :complete)

    assert_event!([:mcp_ex, :subscription, :close], fn measurements, metadata ->
