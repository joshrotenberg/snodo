defmodule Snodo.Extensions.Tasks.Runner do
  @moduledoc """
  ...
  `Snodo.Extensions.Tasks.Work.idempotency_key` for external side effects.
  ...
  application-owned `Snodo.Extensions.Tasks.WorkExecutor` resolves persisted work
  on both the initial claim and every recovery claim.

  Configure `:instrumentation` with an `Snodo.Instrumentation` sink to observe
  ...
  defp maybe_schedule_retry(
    ...
    with false <- Snodo.Extensions.Tasks.Task.terminal?(snapshot.task),
         %{
