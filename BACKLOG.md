...
| Item | Status |
|---|---|
| 2. `:snodo_plug` | **Done.** `integrations/plug`, 17 tests, example 21. |
...
## 1. `Snodo.Client.direct/1` — S, one sitting

The library has a server and no client; every example and the README reach
for the test-only `Snodo.Test` and hand-parse wire maps. Scope EXACTLY per
WHERE-TO-NEXT "First PR": `lib/snodo/client.ex` with `direct/1`, `discover/1`,
`list_tools/1`, `list_resources/1`, `list_resource_templates/1`,
`list_prompts/1`, `call_tool/3`, `read_resource/2`, `get_prompt/3` —
envelope built the way `Snodo.Test.put_protocol_metadata/4` does, dispatched
via `Snodo.Server.dispatch/3`, decoded to structs / `%Snodo.Error{}`. No stdio,
...
Acceptance: tests > 272 and green with warnings-as-errors; `mix snodo.contract`
...
longer contains the string `Snodo.Test`; `lib/snodo/test.ex` untouched.

## 2. `:snodo_plug` sibling package — M

Readiness-plan milestone 3 and the target app's named gap. The pure adapter
already exists (`Snodo.Transport.StreamableHTTP.prepare/3` / `execute/3`).
...
## 3. `Snodo.Client` transports (stdio, http) — S/M, after 1
...
Extend the proven `Snodo.Tool.Simple` precedent: `Snodo.Resource.Simple`,
`Snodo.Prompt.Simple`, inline `tool "name" do ... end` / `resource ... do`
blocks in `Snodo.Server` (macro defines the module underneath — identical
...
non-goals from the assessment: no core edits (`Snodo.Router`,
`Snodo.Server.dispatch/3`, dialects untouched); low level stays the public
...
Sibling package behind the existing `Snodo.Schema.Validator` boundary (24
...
One paragraph: "`snodo` speaks MCP `2026-07-28` only; use a client that
...
package `:snodo_legacy` with its own conformance lane. Precondition
